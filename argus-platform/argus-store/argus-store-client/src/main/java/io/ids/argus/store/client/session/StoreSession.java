...
public abstract class StoreSession<T extends AbstractStub<T>> implements AutoCloseable,
    ...
    protected StoreSession(ManagedChannel channel) {
        ...
        try {
            ...
            throw new ArgusStoreException(StoreError.ERROR_SESSION_ID);
        }
    ...
    private ClientInterceptor createRequestHeader() {
        ...
        Metadata.Key<String> requestId =
                Metadata.Key.of(GrpcContext.HEADER_REQUEST_ID,
    ...
    private ClientInterceptor createSessionHeader() {
        ...
        Metadata.Key<byte[]> typeHeader =
                Metadata.Key.of(GrpcContext.HEADER_SESSION_TYPE,
    ...
    private <M extends com.google.protobuf.Message> Any send(M message) {
        ...
        try {
            ...
            if (!pass) {
                throw new ArgusStoreException(StoreError.TIME_OUT);
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ArgusStoreException(StoreError.ERROR);
        }
        return msg.response;
    }
